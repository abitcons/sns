...
export default function CareersSection() {
    return (<>
      <Helmet>
        ...
        <meta name="description" content="Join Smart National Solutions and drive digital transformation in Saudi Arabia. Discover our exciting benefits, growth opportunities, and open positions."/>
      </Helmet>
  ...
  <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative">
    <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className="space-y-6">
      <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="inline-flex items-center px-4 py-2 bg-gradient-to-r from-[#36a0d0]/10 to-[#6ec6ab]/10 rounded-full">
      ...
      <motion.p variants={fadeIn} className="text-lg text-gray-600 max-w-3xl mx-auto mb-12">
    ...
    <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
      {benefits.map((benefit, index) => (<motion.div key={index} variants={fadeIn} whileHover={{ scale: 1.05, y: -5 }} className="group relative bg-white p-6 rounded-xl shadow-md text-center transition-all duration-300 hover:shadow-xl border border-gray-100">
          ...
          <div className="relative">
            ...
            <div className="mb-6 w-24 h-24 mx-auto bg-white rounded-full shadow-lg group-hover:shadow-xl transition-shadow duration-300">
              <div className="w-full h-full flex items-center justify-center bg-white rounded-full">
                <img src={benefit.icon} alt={`${benefit.title} Icon`} className="w-14 h-14 object-contain"/>
              </div>
            </div>
      ...
      <section className="py-20 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer}>
            ...
            <motion.a href={careersData.portalLink} target="_blank" rel="noopener noreferrer" variants={fadeIn} className="inline-flex items-center px-6 py-3 text-white bg-primary rounded-lg shadow-md hover:bg-primary/90 transition">
