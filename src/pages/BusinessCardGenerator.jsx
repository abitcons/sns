...
END:VCARD`;
    ...
    const downloadAsSVG = async () => {
        ...
        const frontClone = frontSvg.cloneNode(true);
        const backClone = backSvg.cloneNode(true);
    ...
    const downloadAsPDF = async () => {
        ...
        try {
            ...
            // Add images to PDF
            const addPageToDoc = (canvas, isFirstPage = true) => {
                if (!isFirstPage) {
                    pdf.addPage([TOTAL_HEIGHT, TOTAL_WIDTH]);
                }
                pdf.addImage(canvas.toDataURL('image/png', 1.0), 'PNG', BLEED, BLEED, CARD_WIDTH, CARD_HEIGHT, undefined, 'FAST');
                // Add crop marks
                pdf.setDrawColor(0);
            ...
            <div className="grid grid-cols-3 gap-8">
                ...
                <div className="col-span-1">
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div className="flex gap-4">
                            <button onClick={downloadAsSVG} type="button" className="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">
                                Download SVG
                            </button>
                            <button onClick={downloadAsEPS} type="button" className="px-4 py-2 bg-green-500 text-white rounded hover:bg-green-600">
                                Download EPS
                            </button>
                            <button onClick={downloadAsPDF} type="button" className="px-4 py-2 bg-red-500 text-white rounded hover:bg-red-600">
                                Download PDF
                            </button>
                        </div>
                    </form>
                ...
                <div ref={backCardRef} className="w-[350px] h-[200px] bg-white rounded shadow-lg relative mb-6">
                    <svg width="350" height="200" viewBox="0 0 350 200" xmlns="http://www.w3.org/2000/svg">
                        ...
                        {/* Centered Logo */}
                        <image href="/Logo/SNS-logo-white.svg" x="125" y="75" width="100" height="50" preserveAspectRatio="xMidYMid meet"/>
                    </svg>
                </div>
                ...
                <div ref={frontCardRef} className="w-[350px] h-[200px] bg-white rounded shadow-lg relative mb-6">
                    <svg width="350" height="200" viewBox="0 0 350 200" xmlns="http://www.w3.org/2000/svg">
                        ...
                        {/* Logo */}
                        <image href="/Logo/SNS logo SVG SVG-01.svg" x="220" y="20" width="110" height="60" preserveAspectRatio="xMidYMid meet"/>

                        {/* QR Code */}
                        <foreignObject x="220" y="90" width="110" height="110">
                            <QRCodeSVG value={generateQRData()} size={110} level="Q" fgColor="#36a0d0" bgColor="white"/>
                        </foreignObject>
                    </svg>
                </div>
            </div>
