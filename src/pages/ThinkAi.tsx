...
// Define types for our visibility state
      ...
      <section className="relative py-20 bg-gradient-to-r from-[#36a0d0] via-[#5bb6bb] to-[#6ec6ab] text-white text-center">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-4">
            <a 
              href="https://thinkai.sa/"
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center justify-center px-8 py-4 bg-white text-[#36a0d0] rounded-full font-semibold text-lg hover:bg-opacity-90 transition-all"
            >
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center justify-center px-8 py-4 border-2 border-white text-white rounded-full font-semibold text-lg hover:bg-white/10 transition-all"
            >
              Contact Us
            </a>
          </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
              ...
              <div id="product-features" data-animate className={`grid grid-cols-2 gap-6 transform transition-all duration-700 ${isVisible['product-features'] ? 'translate-x-0 opacity-100' : 'translate-x-10 opacity-0'}`}>
                {productFeatures.map((feature, index) => (
                  <div key={index} className="group p-6 bg-white rounded-xl shadow-lg hover:shadow-xl transition-all hover:-translate-y-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {challengeSolutions.map((item, index) => (
              <div 
                ...
                id={`challenge-${index}`}
                data-animate
                className={`group relative bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20 hover:bg-white/20 transition-all transform ${
...
<section className="py-20 bg-white">
  <div className="max-w-7xl mx-auto px-4">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
      {toolboxFeatures.map((feature, index) => (
        <div
          key={index}
          id={`toolbox-${index}`}
          data-animate
          className={`group h-full flex flex-col relative p-6 bg-gradient-to-br from-gray-50 to-white rounded-xl shadow-lg hover:shadow-xl transition-all transform ${
            isVisible[`toolbox-${index}`] ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'
          }`}
        >
...
<section className="py-20 bg-gradient-to-b from-white to-gray-50">
  <div className="max-w-7xl mx-auto px-4">
    ...
    <div className="relative">
      ...
      <div className="hidden md:block absolute top-[50%] h-[2px] bg-[#36a0d0] z-0"
           style={{
             left: 'calc(12.5% + 1rem)',
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
        {implementationSteps.map((step, index) => (
          <div
            ...
            id={`step-${index}`}
            data-animate
            className={`relative transform transition-all duration-700 ease-out h-full
