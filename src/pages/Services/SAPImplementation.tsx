...
const SAPImplementation: FC = () => {
  ...
  interface Service {
    title: string;
    description: string;
    icon: string; // Using string for icon paths; adjust if a specific icon type is used
  }
          ...
          {phases.map((phase, index) => (
            <React.Fragment key={index}>
              ...
              {index < phases.length - 1 && (
                <ArrowRight className="w-6 h-6 text-white animate-pulse flex-shrink-0" />
              )}
            </React.Fragment>
          ))}
        </div>
