...
const ManagementTeam = () => {
    ...
    return (<section className="py-24 bg-gradient-to-br from-[#36a0d0] via-[#5bb6bb] to-[#6ec6ab]">
              ...
              <div onClick={() => setSelectedMember(member)} className="bg-white rounded-2xl shadow-lg 
                ...
                <div className="h-32 p-6">
                  <div className="relative inline-block">
                    <img src={member.imageUrl} alt={member.name} className="w-24 h-24 rounded-xl object-cover shadow-md"/>
                ...
                <div className="px-6 pb-6">
                  ...
                  <div className="pt-4 mt-4 border-t border-gray-100">
                    <div className="flex flex-row gap-2 whitespace-nowrap overflow-hidden">
                      {member.expertise.split(', ').slice(0, 2).map((skill) => (<span key={skill} className="bg-[#36a0d0]/10 text-[#36a0d0] px-3 py-1 rounded-full text-sm font-medium shrink-0">
                          {skill}
                        </span>))}
                    </div>
                  </div>
      ...
      {selectedMember && (<div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setSelectedMember(null)}>
          <div onClick={(e) => e.stopPropagation()} className="bg-white rounded-2xl p-8 max-w-2xl w-full text-gray-900 relative shadow-2xl">
            <button onClick={() => setSelectedMember(null)} className="absolute top-6 right-6 bg-gray-100 hover:bg-gray-200 rounded-full p-2 transition-colors duration-200">
              ✕
            </button>

            <div className="flex flex-col md:flex-row md:items-start gap-8">
              <div className="flex-shrink-0">
                <img src={selectedMember.imageUrl} alt={selectedMember.name} className="w-32 h-32 rounded-xl shadow-lg object-cover border-4 border-[#36a0d0]"/>
              </div>
              ...
              <div className="flex-grow">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedMember.expertise.split(', ').map((skill) => (<span key={skill} className="bg-[#36a0d0]/10 text-[#36a0d0] px-4 py-2 rounded-full text-sm font-medium">
                        {skill}
                      </span>))}
                  </div>
                </div>
