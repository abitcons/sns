...
export default function FooterSection() {
    ...
    return (<footer className="relative bg-gradient-to-r from-primary via-secondary to-accent">
      <div className="max-wfull mx-auto px-4 sm:px-6 lg:px-8 py-16">
        {/* First section remains the same */}
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className="grid grid-cols-1 md:grid-cols-3 gap-16">
          {/* Column 1: About */}
          <motion.div variants={fadeIn} className="flex flex-col items-start space-y-6">
            <Link to="/" className="block">
              <motion.img src="/Logo/SNS-logo-white.svg" alt={t('footer.logo.alt')} className="h-16 w-auto" whileHover={{ scale: 1.1 }} transition={{ type: 'spring', stiffness: 300 }}/>
            </Link>
            ...
            <div className="flex items-center space-x-8 mt-4">
              <img src="/Logo/SAP Gold partner .svg" alt={t('footer.sap_gold_partner.alt')} className="h-12 w-auto"/>
              <img src="/Logo/SAP Certified-white .png" alt={t('footer.sap_certified.alt')} className="h-12 w-auto" style={{ filter: 'brightness(0) invert(1)' }}/>
            </div>
          </motion.div>
  ...
  <div className={`flex flex-col md:flex-row justify-center items-center space-y-4 md:space-y-0 ${isRtl ? 'md:space-x-reverse' : ''} md:space-x-8`}>
    <Link to="/news" className="w-full md:w-auto px-6 py-3 bg-accent text-white rounded-lg hover:bg-accent-light transition-all duration-200 text-center">
      {t('footer.updates.news_button')}
    </Link>
    <Link to="/blog" className="w-full md:w-auto px-6 py-3 bg-accent text-white rounded-lg hover:bg-accent-light transition-all duration-200 text-center">
        ...
        <motion.div variants={fadeIn} className="mt-12 border-t border-white/20 pt-8">
          <div className={`flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0 ${isRtl ? 'md:space-x-reverse' : ''} md:space-x-6`}>
            ...
            <div className={`flex ${isRtl ? 'space-x-reverse' : ''} space-x-6`}>
              <Link to="/privacy-policy-2" className="text-sm text-white/90 hover:text-white transition-all duration-200">
                {t('footer.legal.privacy')}
              </Link>
              <Link to="/terms-and-conditions2" className="text-sm text-white/90 hover:text-white transition-all duration-200">
