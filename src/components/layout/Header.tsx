...
import { Phone, ChevronRight, ChevronDown, X, Menu } from "lucide-react";
...
const Header: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const initializeLanguage = () => {
      // Set language and direction immediately without animation or visible transition
      document.documentElement.lang = defaultLang;
      document.documentElement.dir = defaultLang === "ar" ? "rtl" : "ltr";
      
      // Change language silently
  ...
  <div className="flex items-center gap-6 flex-shrink-0 lg:mr-24"> {/* Added lg:mr-24 for extra space */}
    {/* Main Logo */}
    <motion.div 
    ...
    >
      ...
      />
        ...
        <div className="hidden lg:flex lg:items-center lg:gap-x-24 flex-1 justify-end w-full"> {/* Added w-full */}
          <div className="flex lg:items-center lg:gap-x-6"> {/* Increased gap from lg:gap-x-1 to lg:gap-x-6 */}
            {navigation.map((item) => {
                        ...
                        >
                          {item.submenu.map((subItem, index) => (
                            ...
                            >
                              <Link
                                to={subItem.href}
                                className="block px-4 py-2 text-gray-700 hover:bg-primary/5 hover:text-primary transition-all duration-200"
                              >
                                <motion.div
...
{/* Language Icon */}
<img 
    src="/Language-switcher/language.gif" 
  ...
  {mobileMenuOpen && (
    <>
      ...
      <motion.div
        ...
        className={`fixed inset-y-0 ${isRtl ? 'left-0' : 'right-0'} w-full max-w-sm bg-white shadow-lg z-50 lg:hidden overflow-y-auto`}
      >
        {/* Mobile Menu Header with both logos */}
        <div className="flex items-center justify-between p-8 border-b"> {/* Increased from p-6 to p-8 */}
          <div className="flex items-center gap-8"> {/* Increased from gap-6 to gap-8 */}
            {/* Main Logo */}
            <Logo variant="default" className="h-14" /> {/* Increased from h-12 to h-14 */}
            {/* Separator Line */}
...
<div className="px-8 py-6 border-b">
  <div className="flex items-center gap-3 mb-4">
    ...
    />
    <span className="text-sm font-medium text-gray-500">{t('header.language_selector')}</span>
  </div>
  <div className="flex gap-2 bg-gray-50 rounded-2xl p-1">
    ...
    >
      <span>عربي</span>
        ...
        <nav className="p-8"> {/* Increased from p-6 to p-8 */}
          <motion.ul 
            className="space-y-6" 
            initial="closed"
            animate="open"
  ...
  onClick={() => {
    if (item.submenu) {
      ...
      // For items without submenu, navigate and close menu
      navigate(item.href);
      setMobileMenuOpen(false);
    }
  }}
  ...
  {item.submenu && (
    ...
    <AnimatePresence initial={false}>
      {activeDropdown === item.name && (
        ...
        >
          <div className={`${isRtl ? 'mr-8' : 'ml-8'} mt-4 space-y-3`}>
            {item.submenu.map((subItem, index) => (
              <motion.div
                key={subItem.href}
              ...
              >
                <Link
                  to={subItem.href}
                  className="block px-6 py-4 text-gray-600 hover:text-primary rounded-lg 
                    hover:bg-primary/5 transition-all duration-200 relative group"
                  onClick={() => setMobileMenuOpen(false)}
                >
                  <motion.div
                    className="flex items-center justify-between"
                  ...
                  >
                    ...
                    <ChevronRight 
                      className={`w-4 h-4 opacity-0 group-hover:opacity-100 
                        transition-opacity duration-200 ${isRtl ? 'rotate-180' : ''}`} 
                    />
                  </motion.div>
                </Link>
              </motion.div>
