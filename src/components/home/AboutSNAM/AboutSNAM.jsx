...
export default function AboutSNAM() {
    ...
    const solutions = Array.isArray(t("about_snam.solutions", { returnObjects: true }))
        ? t("about_snam.solutions", { returnObjects: true })
        : [];
    // ✅ Adjust arrow direction based on language
    const isArabic = i18n.language === "ar";
    const arrowDirection = isArabic ? "ml-2 rotate-180" : "ml-2";
    return (<section id="about-snam">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <motion.div className="text-gray-800">
              ...
              <motion.div className="mt-6 p-4 bg-white/80 backdrop-blur-sm rounded-lg shadow-md inline-flex items-center gap-4">
                <img src="/Logo/SAP_Qualified_PartnerPackageSolution_logo.svg" alt="SAP Partner Badge" className="h-16 w-auto"/>
                <div>
            ...
            <motion.div className="relative group">
              <div className="relative rounded-2xl overflow-hidden shadow-2xl">
                <img src="/images/aboutsnam.png" alt="SNAM Solutions Overview" className="w-full h-full object-cover transform group-hover:scale-105 transition-transform duration-700"/>
              </div>
            </motion.div>
          </div>
        </div>
      ...
      <section className="relative py-16 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <motion.div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {solutions.map((solution, index) => (<motion.div key={index} className="group relative bg-white rounded-lg p-6 transition-all duration-300 shadow-lg hover:shadow-xl" variants={{
                ...
                <div className="relative">
                  ...
                  <div className="mb-4 h-12 flex justify-center">
                    <img src={solution.logo} alt={`${solution.title} Logo`} className="h-full object-contain"/>
                  </div>
                  ...
                  <ul className="space-y-2">
                    {solution.features.map((feature, idx) => (<li key={idx} className="flex items-center text-gray-600 text-sm group-hover:text-gray-700 transition-colors duration-300">
