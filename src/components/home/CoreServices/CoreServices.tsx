        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {services.map((service, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
              variants={fadeIn}
              className="relative bg-white p-8 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 flex flex-col items-center text-center group"
              style={{ minHeight: "300px" }}
            >
              {/* Icon Section with Hover Interaction */}
              <div
                className="relative flex items-center justify-center h-24 w-24 mx-auto mb-6 bg-white rounded-full shadow-md p-4 transition-transform duration-300 group-hover:scale-110 group-hover:shadow-lg"
              >
                <img
                  src={service.icon}
                  alt={t(`core_services.services.${service.key}.title`)}
              ...
              {/* Gradient Hover Line */}
              <div
                className="absolute bottom-6 left-6 right-6 h-1 bg-gradient-to-r from-[#36a0d0] via-[#5bb6bb] to-[#6ec6ab] scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left"
              />
            </motion.div>
          ))}
        </div>
