...
const PartnersSection = () => {
    ...
    return (<section className="relative overflow-hidden bg-gradient-to-br from-gray-50 via-white to-gray-50">
      ...
      <div className="bg-gradient-to-r from-[#36a0d0] via-[#5bb6bb] to-[#6ec6ab] py-10 text-center text-white">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer}>
          <motion.h2 variants={fadeIn} className="text-4xl font-extrabold mb-4">
      ...
      <div className="py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="relative">
            {/* Navigation Buttons */}
            <button onClick={() => handleSlideChange('prev')} className="absolute left-0 top-1/2 transform -translate-y-1/2 -translate-x-4 z-10 bg-white/80 hover:bg-white p-2 rounded-full shadow-lg backdrop-blur-sm transition-all duration-300 group" aria-label="Previous slide">
              <ChevronLeft className="w-6 h-6 text-[#36a0d0] group-hover:scale-110 transition-transform duration-300"/>
            </button>
            <button onClick={() => handleSlideChange('next')} className="absolute right-0 top-1/2 transform -translate-y-1/2 translate-x-4 z-10 bg-white/80 hover:bg-white p-2 rounded-full shadow-lg backdrop-blur-sm transition-all duration-300 group" aria-label="Next slide">
            ...
            <div className="overflow-hidden py-8">
              <AnimatePresence mode="wait">
                <motion.div key={currentSlide} initial={{ opacity: 0, x: 100 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -100 }} transition={{ duration: 0.5 }} className="flex justify-center gap-8">
                  {getCurrentPartners().map((partner, index) => (<motion.div key={partner.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.2 }} className="group relative w-full max-w-sm">
                      <div className="relative bg-white rounded-2xl p-8 h-96 flex flex-col transition-all duration-500 transform hover:-translate-y-2">
                        ...
                        <div className="relative h-32 flex items-center justify-center">
                          <div className="w-36 h-16 relative flex items-center justify-center">
                            <img src={partner.logo} alt={`${partner.name} logo`} className="absolute inset-0 w-full h-full object-contain transition-transform duration-500 group-hover:scale-105" onError={(e) => {
            ...
            <div className="flex justify-center gap-2 mt-8">
              {Array.from({ length: totalSlides }).map((_, index) => (<button key={index} onClick={() => handleDotClick(index)} aria-label={`Go to slide ${index + 1}`} className={`w-8 h-1 rounded-full transition-all duration-300 ${currentSlide === index
