...
const Aboutus = () => {
    ...
    return (<section className="relative py-20 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <motion.div variants={slideInRight} className="flex flex-col">
            ...
            <div className="grid grid-cols-3 gap-4 mb-8">
              {iconBoxes.map((item) => (<motion.div key={item.key} whileHover={{ scale: 1.05, backgroundColor: "#f8fafc" }} className="p-4 bg-white rounded-xl shadow-lg transition-all duration-300">
            ...
            <a href="/about" className={`group inline-flex items-center px-6 py-3 rounded-lg bg-gradient-to-r from-[#36a0d0] to-[#5bb6bb] text-white hover:opacity-90 transition-all duration-300 cursor-pointer z-10 relative ${isRtl ? 'flex-row-reverse self-start' : 'self-start'}`}>
          ...
          <motion.div variants={slideInRight} className="hidden lg:block">
            <div className="relative">
              ...
              <img src="/images/about-sns.jpg" alt={t('about.imageAlt')} className="rounded-lg shadow-xl relative z-10"/>
            </div>
          </motion.div>
        </motion.div>
