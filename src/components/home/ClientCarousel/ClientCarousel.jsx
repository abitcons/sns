...
function ClientCard({ name, logo }) {
    return (<div className="flex-shrink-0 w-[220px] h-[140px] bg-white rounded-lg shadow-lg flex items-center justify-center">
      <img src={logo} alt={name} className="max-w-[80%] max-h-[80%] object-contain" loading="lazy"/>
    </div>);
}
      ...
      <div className="py-20 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="relative">
            <button onClick={scrollLeft} className={`absolute -left-4 top-1/2 -translate-y-1/2 z-10 p-2 rounded-full bg-white shadow-lg text-primary hover:text-white hover:bg-primary transition-colors ${!canScrollLeft ? 'opacity-50 cursor-not-allowed' : ''}`} disabled={!canScrollLeft} aria-label="Scroll left">
              <ChevronLeft className="h-6 w-6"/>
            </button>

            <div ref={containerRef} className="overflow-hidden px-4">
              <motion.div animate={{ x: 0 }} transition={{ duration: 0.5 }} className="flex gap-8 py-8">
                {clients.map((client, index) => (<ClientCard key={index} {...client}/>))}
              </motion.div>
            </div>

            <button onClick={scrollRight} className={`absolute -right-4 top-1/2 -translate-y-1/2 z-10 p-2 rounded-full bg-white shadow-lg text-primary hover:text-white hover:bg-primary transition-colors ${!canScrollRight ? 'opacity-50 cursor-not-allowed' : ''}`} disabled={!canScrollRight} aria-label="Scroll right">
