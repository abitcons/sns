...
import { ArrowRight, Brain, Shield, BarChart, Database, Play, Pause } from "lucide-react";
import { useState, useRef } from "react";
import { useTranslation } from "react-i18next";

const ThinkAISection = () => {
  const { t, i18n } = useTranslation("ThinkAI.Home");
  ...
  const features = Array.isArray(t("thinkai.features", { returnObjects: true }))
    ? (t("thinkai.features", { returnObjects: true }) as Array<{ title: string; description: string }>)
    : [];
  ...
  return (
    <section id="ThinkAISection" className="py-24 bg-gradient-to-br from-gray-50 via-white to-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header */}
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-bold mb-6">{t("thinkai.title")}</h2>
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">{t("thinkai.description")}</p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            ...
            <div className="relative w-full min-h-[500px] lg:min-h-[600px]" onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
              <motion.div className="relative z-10 w-full h-full rounded-3xl overflow-hidden shadow-2xl" whileHover={{ scale: 1.02 }} transition={{ duration: 0.3 }}>
                <video ref={videoRef} className="w-full h-full object-cover object-[100%_center]" autoPlay muted loop playsInline>
                  <source src="/hero video/ThinkAI.mp4" type="video/mp4" />
                ...
                <motion.div className="absolute inset-0 bg-gradient-to-t from-black/50 via-transparent to-transparent" initial={{ opacity: 0 }} animate={{ opacity: isHovered ? 1 : 0 }} transition={{ duration: 0.3 }}>
                  ...
                  <div className="absolute bottom-6 left-20 right-6">
                    <h3 className="text-white text-lg font-medium">{t("thinkai.video.title")}</h3>
                    <p className="text-white/80 text-sm">{t("thinkai.video.description")}</p>
                  </div>
                </motion.div>
              </motion.div>
            </div>
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="lg:col-span-6 space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {features.map((feature, index) => (
                <motion.div key={index} whileHover={{ scale: 1.03 }} className="bg-white p-6 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300">
                  ...
                  {index === 0 && <Shield className="h-12 w-12 text-[#36a0d0] mb-4" />}
                  {index === 1 && <Brain className="h-12 w-12 text-[#36a0d0] mb-4" />}
                  {index === 2 && <BarChart className="h-12 w-12 text-[#36a0d0] mb-4" />}
                  {index === 3 && <Database className="h-12 w-12 text-[#36a0d0] mb-4" />}
            ...
            {/* CTA Button */}
            <div className="pt-6">
              <Link to="/ThinkAi" className="inline-flex items-center px-8 py-4 bg-gradient-to-r from-[#36a0d0] to-[#6ec6ab] text-white rounded-lg hover:from-[#5bb6bb] hover:to-[#2980b9] transition-all duration-300 shadow-lg hover:shadow-xl group">
                <span className="text-lg">{t("thinkai.cta.title")}</span>
