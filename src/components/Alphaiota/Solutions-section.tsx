...
const SolutionsSection: React.FC = () => {
  ...
  return (
    <section className="py-24 relative overflow-hidden" 
             style={{ background: 'linear-gradient(to bottom right, #ffffff, #f0f9fb)' }}>
          ...
          {solutions.map((solution, index) => (
            <motion.div
              ...
              className={`relative group cursor-pointer rounded-2xl p-6 transition-all duration-300 ${
                activeCard === index ? 'scale-105' : 'hover:scale-[1.02]'
              }`}
              style={{ 
              ...
              <div className="relative z-10">
                <div className="flex items-center gap-4 mb-4">
                  <div className="p-3 rounded-xl"
                       style={{ backgroundColor: 'rgba(54, 160, 208, 0.1)' }}>
                  ...
                  {activeCard === index && (
                    ...
                    >
                      <ul className="space-y-2 mt-4 border-t pt-4"
                          style={{ borderColor: 'rgba(54, 160, 208, 0.2)' }}>
                        {solution.features.map((feature, idx) => (
                          <li key={idx} className="flex items-center gap-2 text-sm text-gray-600">
                            <div className="w-1.5 h-1.5 rounded-full"
                                 style={{ backgroundColor: '#5bb6bb' }}></div>
                            {feature}
                          </li>
                        ))}
                      </ul>
