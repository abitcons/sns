...
const NewsPostCard: React.FC<NewsPostCardProps> = ({ post }) => {
  ...
  return (
    <div className="group h-full bg-white rounded-xl shadow-lg overflow-hidden transition-all duration-300 hover:shadow-2xl">
      ...
      <div className="flex flex-col flex-grow p-6">
        ...
        <div className="flex items-center justify-between gap-4 mt-auto pt-4 border-t border-gray-100">
          <Link
            to={`/news/${post.slug}`}
            className="inline-flex items-center px-4 py-2 bg-gradient-to-r from-[#36a0d0] to-[#6ec6ab] text-white rounded-lg text-sm font-medium shadow-md hover:from-[#2f8bb4] hover:to-[#5ba892] transition-all duration-300"
          >
            Read More
            <svg 
          ...
          <button
            ...
            className="px-4 py-2 bg-gray-50 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-100 transition-colors duration-300 flex items-center"
          >
            {isExpanded ? (
              <>
                View Less
