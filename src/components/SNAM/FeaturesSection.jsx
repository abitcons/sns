...
const FeatureCard = ({ icon, title, description, delay }) => {
    ...
    return (<motion.div className="bg-white rounded-lg p-6 shadow-xl cursor-pointer relative overflow-hidden group" initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay }} onHoverStart={() => setIsHovered(true)} onHoverEnd={() => setIsHovered(false)} onMouseDown={() => setIsPressed(true)} onMouseUp={() => setIsPressed(false)} onMouseLeave={() => setIsPressed(false)} style={{
            transform: isPressed ? 'scale(0.98)' : 'scale(1)',
            transition: 'transform 0.2s'
        }}>
      {/* Themed gradient background on hover */}
      <motion.div className="absolute inset-0 bg-gradient-to-r from-[#36a0d0] via-[#5bb6bb] to-[#6ec6ab] opacity-0 group-hover:opacity-10" initial={false} animate={{
            opacity: isHovered ? 0.15 : 0,
        }} transition={{ duration: 0.3 }}/>

      <div className="relative flex flex-col items-center text-gray-800">
        {/* Icon container with themed shadow */}
        <motion.div className="bg-white rounded-full p-4 shadow-lg mb-6" animate={{
          ...
          <img src={`/Icons/${icon}.gif`} alt={`${title} Icon`} className="h-12 w-12"/>
        </motion.div>

        {/* Title with themed color transition */}
        <motion.h3 className="text-xl font-bold mb-3 text-center" animate={{
        ...
        <motion.p className="text-gray-600 text-center" animate={{
...
const FeaturesSection = () => {
    ...
    return (<section className="py-24 bg-gradient-to-r from-[#36a0d0] via-[#5bb6bb] to-[#6ec6ab] relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        <motion.div initial={{ opacity: 0, y: -20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16">
          <motion.h2 className="text-4xl md:text-5xl font-bold text-white mb-6" whileInView={{
            opacity: [0, 1],
            y: [-20, 0]
        }} transition={{ duration: 0.8 }}>
            Why Choose SNAM Solutions
          </motion.h2>
          <motion.p className="text-xl text-white/90 max-w-3xl mx-auto" whileInView={{
            opacity: [0, 1],
            y: [20, 0]
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {features.map((feature, index) => (<FeatureCard key={feature.icon} icon={feature.icon} title={feature.title} description={feature.description} delay={index * 0.1}/>))}
        </div>
      </div>
    </section>);
};
