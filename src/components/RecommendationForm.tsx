...
const RecommendationForm = () => {
    ...
    const [recommendationResult, setRecommendationResult] = useState<{
        primarySolution: typeof solutionsData[0];
        complementarySolutions: typeof solutionsData[0][];
    } | null>(null);
    const [error, setError] = useState<string>('');

    const findRecommendation = (input: string): {
        primarySolution: typeof solutionsData[0];
        complementarySolutions: typeof solutionsData[0][];
    } | null => {
        ...
        const complementarySolutions = solutionsData.filter(
            sol => sol.integrations.includes(primarySolution.Name)
        ).map(sol => {
            let score = 0;
        ...
        <div className="p-8 max-w-4xl mx-auto">
            ...
            {recommendationResult && (
                <div className="mt-8">
                    ...
                    {recommendationResult.complementarySolutions.map((sol, index) => (
                        <p key={index}>{sol.Name}</p>
                    ))}
                </div>
            )}
        </div>
